...
class IRC_Server:
    ...
    def __init__(self, host, port, nick, channel , password =""):
        ...
        self.command = ""
        self.Zecora_cooldown = False
        self.Zecora_lock = False
    ...
    def listen(self):
        while self.is_connected:
            ...
            if str(recv).find ( "PRIVMSG" ) != -1:
                ...
                if ( str(irc_user_message[0]) != "!" ):
                    self.usermessage = str(irc_user_message)
                    self.process_message(irc_user_nick, ( (str(recv)).split()[2] ) )
    ...
    def process_command(self, user, channel):
        ...
        if (user == root_admin ) or (user == moderator):
 ...
 s              
        ...
        if ( len(command) == 1):      
            ...
            if (command[0] == "salut"):
                self.send_message_to_channel( ("Coucou, " + user), channel )
               
            if (command[0] == "chut"):
                self.send_message_to_channel( ("D'accord, " + user), channel )
                self.Zecora_lock = True
            ...
            if (command[0] == "doom"):
                self.send_message_to_channel( ("You\'re doomed, " + str(command[1])), channel )
           
    def process_message(self, user, channel):
        usermessage = (self.usermessage).lower()
       
        if (usermessage == "salut"):
            self.send_message_to_channel( ("Coucou, " + user), channel )
       
        if str(usermessage).find ( "zecora" ) and (user != "Zecora_bot" or "zecora_bot") and ( self.Zecora_lock == False):
            if (self.Zecora_cooldown != False):
                time.sleep(10)
                self.Zecora_cooldown = False
            else :    
                self.Zecora_cooldown = True
                self.send_message_to_channel( ("On m'appelle ?"), channel )
           
# Here begins the main programs flow:
 
test = IRC_Server("irc.freenode.com", 6667, "Zecora_bot", "#test_berrypunch")
